...
readAllObs <- function(obs='numberInfected', nStat = 100){
  #read all specified files
  evalTheseIdx <- which(nameList == obs)
  ...
  for (idx in c(1:length(evalTheseIdx))){
    ...
    if(ncol(df) == (nStat +1)){
      ...
      usedIdx <- c(usedIdx, idx)
    }
  usedParams <- evalThisParams[,usedIdx]
  }
  return(list(usedParams, dfList))
}
